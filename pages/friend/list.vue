<template>
	<view class="page-body">
		...
		<vk-data-table ref="table1" :action="table1.action" :columns="table1.columns" :query-form-param="queryForm1"
			...
			<template v-slot:images="{ row, column, index }">
				<view v-if="row.images">
					<el-image v-for="(image,index) in row.images" :key="index" style="width: 40px;height: 40px;margin-left: 10px;"
						:src="image" :preview-src-list="row.images">
					</el-image>
				</view>
				<view v-if="!row.images">
					<view >
	...
	export default {
		data() {
			...
			return {
				...
				table1: {
					// 表格数据请求地址
					action: "admin/friend/sys/getList",
					// 表格字段显示规则
					columns: [{
							key: "_id",
							title: "id",
		...
		methods: {
			...
			deleteBtn({
				...
				deleteFn({
					action: "admin/friend/sys/delete",
					data:item
				});
			},
		},
